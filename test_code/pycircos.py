...
class Gcircle:
    ...
    def set_garcs(self, start=0, end=360):
        ...
        if self.fig_is_ext:
            #self.ax_base = self.figure.add_axes([0, 0, self.figsize[0], self.figsize[1]])
    ...
    def barplot(self, garc_id, data, positions=None, width=None, raxis_range=(550, 600), rlim=None, base_value=None, facecolor=None, edgecolor="#303030", linewidth=0.0, spine=False):
        # print(" GARC for ID",garc_id)
        # print(dir(self._garc_dict[garc_id]))

        # print("BRITT BARPLOT -garcid,positions,width",garc_id,positions,width)
        start = self._garc_dict[garc_id].coordinates[0]
        end   = self._garc_dict[garc_id].coordinates[-1]
        size  = self._garc_dict[garc_id].size - 1 # DOES NOT ALLOW FOR PARTITIONS OF SIZE 1
        # print(f" ERROR: PARTITIONSIS {garc_id} is SIZE 1",size)

        # print("-garcid,start,end,size",garc_id, round(start,2),round(end,2),round(size,2))
        positions_all = np.linspace(start, end, len(data), endpoint=True)
        if positions is None:
            positions = positions_all
        else:
